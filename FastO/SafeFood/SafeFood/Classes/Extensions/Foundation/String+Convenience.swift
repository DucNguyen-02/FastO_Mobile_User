...
extension String {
    ...
    /// ```
    /// "LK Hoang".contains(find: "Hoang") // true
    /// "LK Hoang".contains(find: "hoang") // false
    /// ```
    ...
    /// Only contains alphabet (a => z, A => Z, 0 => 9)
    ...
    /// Only contains alphabet (a => z, A => Z)
...
extension String {
    // bỏ dấu, viết hoa chữ cái đầu mỗi từ, bỏ khoảng trắng
    var forSorting: String {
        let simple = folding(options: [.diacriticInsensitive, .widthInsensitive, .caseInsensitive], locale: nil).capitalized(with: NSLocale.current)
